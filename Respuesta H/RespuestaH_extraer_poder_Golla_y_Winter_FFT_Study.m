% Calcula los espectros PSD FFT de los electrodos O1 y O2 para los componentes  fundamentales de cada  segmento de fotoestimulacion 
% y crea archivos '.*xls' con dichos valores. Permite plotear ademas los resultados promediados entre O1 y O2. Guarda los 
...
for gindex = 1:length(group)
    ...
    for index = 1:len
        ...
        try
            ...
            try EEG.comments = []; EEG.history = []; EEG = rmfield( EEG , 'original_EEG' ); catch ME;  end
...
%                 O2_final(index, idx) = 10*log10(spectra_F1(2,1));
                ...
                O1_final(index, idx) = spectra_F1(1,1); % Abs power FFT O1.
                O2_final(index, idx) = spectra_F1(2,1); % Abs power FFT O2.

                % Agrega datos temporales. Columnas contienen PSD de cada F1, picos y respuesta.
    ...
    try
    writecell(final_matrices{1}, [mother_dir, group{gindex}, ' sin normalizar ', spec_AVG]);
    writecell(final_matrices{2}, [mother_dir, group{gindex}, ' sin normalizar ', spec_O1]) ;
    writecell(final_matrices{3}, [mother_dir, group{gindex}, ' sin normalizar ', spec_O2]) ;
    catch ME
        warning("%s Line %d in '%s'", ME.message,  ME.stack.line, ME.stack.name);
    end
