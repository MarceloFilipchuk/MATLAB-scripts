...
%             EEG2 = pop_rmdat(EEG, {sprintf(label, k)}, [0 9.5] ,0);
            
            % Calcula el espectro para la frecuencia necesitada en el rango de tiempo correspondiente al evento.
            % Usa un overlaping del 50% de la ventana, o sea, 100 datapoints.
            [spectra, freqs] = pop_spectopo(EEG2, 1, [0 9500], 'EEG' ,'percent', 100, 'plot', 'off', 'plotchans', [9, 10], 'overlap', EEG2.srate/2 );
            idx = find(freqs == k);
            
            % Computa poder absoluto para cada canal (O1 y O2)
