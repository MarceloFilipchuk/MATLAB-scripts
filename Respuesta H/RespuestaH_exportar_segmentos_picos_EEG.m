...
for findex = 1:length(filepath)
    ...
    tmpfilepath = strcat(filepath{findex}, '\');
    ...
    folderH = strcat(tmpfilepath, 'H response EEG\');
    folderN = strcat(tmpfilepath, 'N response EEG\');
    ...
    for index = 1:length(eegs)
        EEG = pop_loadset('filename', eegs{index}, 'filepath', tmpfilepath);
