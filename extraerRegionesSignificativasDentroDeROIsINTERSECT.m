...
for findex = 1:length(filepath)
    ...
    for tindex = 1:length(tables)
        ...
        if ~isempty(voxtable)
            ...
            voxtable(~ismember(voxtable.Brodmann_area, buscar), :) = [];
            
            % Itera sobre cada linea de la tabla.
            for vindex = 1:size(voxtable, 1)
                if voxtable.P_value(vindex) < 0.05
%                     tmp{tmpcounter, 1} = strcat(voxtable.Hemisphere{vindex},' - ', 'BA', num2str(voxtable.Brodmann_area(vindex))); % extrae el area de Brodmann 
                    tmp{tmpcounter, 1} = voxtable.Structure{vindex}; % extrae la estructura
    ...
    % Deja solo las estructuras que estan dentro de 
...
writecell(final, strcat(workdir, 'ESTRUCTURAS significativas INTERSECT.xls'));
